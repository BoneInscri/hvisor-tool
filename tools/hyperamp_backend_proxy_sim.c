 ...
 *
 * 编译: gcc -o hyperamp_backend hyperamp_backend_proxy_sim.c
 * shm/hyperamp_linux_shm.c -I./include -lpthread
 *
 ...
 */
static void handle_session_message(HyperampMsgHeader *req_hdr, void *payload,
                                   uint16_t payload_len) {
    ...
    const char *proto_name =
        (sess->protocol == PROXY_PROTO_TCP) ? "TCP" : "UDP";
    ...
    if (hyperamp_linux_send(HYPERAMP_MSG_TYPE_SESS, req_hdr->frontend_sess_id,
                            backend_sess, &resp_sess,
 ...
 */
static void handle_data_message(HyperampMsgHeader *req_hdr, void *payload,
                                uint16_t payload_len) {
    ...
    if (payload_len >= sizeof(HttpRequestHeader)) {
        ...
        if (http_req->method[0] >= 'A' && http_req->method[0] <= 'Z') {
            ...
            printf("[Backend] Simulating network request (not actually "
                   "connecting)...\n");
            ...
            if (hyperamp_linux_send(HYPERAMP_MSG_TYPE_DATA,
                                    req_hdr->frontend_sess_id,
                                    req_hdr->backend_sess_id, fake_response,
            ...
            if (hyperamp_linux_send(HYPERAMP_MSG_TYPE_DATA,
                                    req_hdr->frontend_sess_id,
                                    req_hdr->backend_sess_id, payload,
...
int main(int argc, char *argv[]) {
    ...
    printf("[Backend] Initializing HyperAMP as CONNECTOR (waiting for seL4 to "
           "initialize queues)...\n");
    if (hyperamp_linux_init(0, 0) !=
        HYPERAMP_OK) { // is_creator=0（等待 seL4 初始化）
        printf("[Backend] ERROR: Failed to initialize HyperAMP\n");
        printf("[Backend] Make sure you have root permissions and /dev/mem "
               "access!\n");
        return 1;
    }
    ...
    while (g_running) {
        ...
        // 从 TX Queue 接收消息（非阻塞）
        int ret = hyperamp_linux_recv(&req_hdr, payload, sizeof(payload) - 1,
                                      &payload_len);
        ...
        if (ret == HYPERAMP_OK) {
            ...
            case HYPERAMP_MSG_TYPE_STRGY: // 策略配置
                ...
                printf("[Backend] Received %s message: %s\n",
                       req_hdr.proxy_msg_type == HYPERAMP_MSG_TYPE_DEV
                           ? "DEVICE"
                           : "STRATEGY",
                       (char *)payload);
                break;
