...
struct AmpMsgQueue {
    ...
    uint16_t buf_size; /* 缓冲区队列的缓冲区数量，必须大于 0
                          才算初始化成功，初始状态为 0 */
...
struct MsgQueueOps {
    ...
    int32_t (*push)(struct AmpMsgQueue *msg_queue, uint16_t *head,
                    uint16_t msg_index);
    int32_t (*transfer)(struct AmpMsgQueue *msg_queue, uint16_t *from_head,
                        uint16_t *to_head);
};
...
struct MsgQueueMutexOps {
    int32_t (*mutex_init)(struct MsgQueueMutex *mutex,
