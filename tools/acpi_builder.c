...
static int load_to_hpa(int dev_fd, const void *buf, uint64_t size,
                       uint64_t hpa) {
...
static int append_region(zone_config_t *config, uint64_t hpa, uint64_t gpa,
                         uint64_t size) {
    ...
    log_info("acpi_builder: region HPA=0x%llx GPA=0x%llx size=0x%llx",
             (unsigned long long)hpa, (unsigned long long)gpa,
...
int acpi_build_and_load(int dev_fd, const char *acpi_dir,
     ...
     * 更简单的方式：直接读 XSDT 文件，从其头部拿 GPA 不可行（sysfs 不暴露
     * GPA）。 因此：从 /proc/iomem 解析各表的原始 GPA。 */
    ...
    acpi_rsdp_t rsdp_orig;
    {
        ...
        void *base =
            mmap(NULL, map_sz, PROT_READ, MAP_SHARED, memfd, (off_t)aligned);
        if (base == MAP_FAILED) {
    ...
    uint32_t xsdt_len = 0;
    {
        ...
        void *base =
            mmap(NULL, map_sz, PROT_READ, MAP_SHARED, memfd, (off_t)aligned);
        if (base == MAP_FAILED) {
            log_error("acpi_builder: mmap XSDT hdr failed: %s",
                      strerror(errno));
            close(memfd);
            return -1;
        }
    ...
    uint64_t child_gpas[MAX_ACPI_TABLES];
    {
        ...
        void *base =
            mmap(NULL, map_sz, PROT_READ, MAP_SHARED, memfd, (off_t)aligned);
        if (base == MAP_FAILED) {
            log_error("acpi_builder: mmap XSDT full failed: %s",
                      strerror(errno));
            close(memfd);
            return -1;
        }
    ...
    if (!dir) {
        log_error("acpi_builder: cannot open %s: %s", acpi_dir,
                  strerror(errno));
        return -1;
    }
    struct dirent *ent;
    while ((ent = readdir(dir)) != NULL &&
           sig_map_n < (int)(sizeof(sig_map) / sizeof(sig_map[0]))) {
        if (ent->d_name[0] == '.')
            continue;
        ...
        if (!buf || sz < sizeof(acpi_table_hdr_t)) {
            free(buf);
            continue;
        }
...
#define FIND_SIG(s, out_data, out_size)                                        \
    do {                                                                       \
        (out_data) = NULL;                                                     \
        (out_size) = 0;                                                        \
    ...
    for (int i = 0; i < n_xsdt_entries; i++) {
        ...
        if (!tdata) {
            log_warn(
                "acpi_builder: no sysfs file for sig '%.4s' GPA=0x%llx, skip",
        ...
        if (append_region(config, hpa, cgpa, ALIGN_4K(tsize)) != 0)
            goto err;
        log_info("acpi_builder: [child] %.4s GPA=0x%llx HPA=0x%llx", sig,
                 (unsigned long long)cgpa, (unsigned long long)hpa);
    }

    /* ---- Step 5: 处理 DSDT（FADT offset 0x8C 引用，GPA 可能不在 XSDT 里）----
     */
    {
        uint8_t *facp_data = NULL;
        uint32_t facp_size = 0;
        ...
        if (facp_data && facp_size >= 0x94) {
            ...
            if (dsdt_gpa) {
                ...
                int found = 0;
                for (int i = 0; i < n; i++)
                    if (entries[i].orig_gpa == dsdt_gpa) {
                        found = 1;
                        break;
                    }
                if (!found) {
                    ...
                    if (ddata) {
                        ...
                        e->orig_gpa = dsdt_gpa;
                        e->new_hpa = hpa;
                        if (append_region(config, hpa, dsdt_gpa,
                                          ALIGN_4K(dsize)) != 0)
                            goto err;
                        log_info("acpi_builder: [DSDT] GPA=0x%llx HPA=0x%llx",
                                 (unsigned long long)dsdt_gpa,
    ...
    /* ---- Step 6: 重建 XSDT，GPA 不变，HPA 新分配 ---- */
    {
        ...
        uint8_t *orig_xsdt_data = NULL;
        uint32_t orig_xsdt_size = 0;
        ...
        if (load_to_hpa(dev_fd, xsdt_buf, ALIGN_4K(new_xsdt_size), xsdt_hpa) !=
            0) {
            free(xsdt_buf);
            goto err;
        }
        ...
        if (append_region(config, xsdt_hpa, xsdt_gpa,
                          ALIGN_4K(new_xsdt_size)) != 0)
            goto err;
    ...
    /* ---- Step 7: 重建 RSDP，GPA 不变，HPA 新分配 ---- */
    {
        ...
        if (append_region(config, rsdp_hpa, rsdp_gpa, PAGE_SIZE_4K) != 0)
            goto err;
