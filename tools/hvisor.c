...
static __u64 load_chunked_image_to_memory(const char *path, __u64 load_ipa,
    ...
    for (int i = 0; i < num_memory_regions_ram; i++) {
        ...
        void *virt_addr_chunk =
            mmap(NULL, map_size_chunk, PROT_READ | PROT_WRITE | PROT_EXEC,
...
static int zone_start_from_json_dynamic(const char *json_config_path,
    ...
    strncpy(config->boot_method, boot_method_json->valuestring,
            CONFIG_NAME_MAXLEN - 1);
    ...
    //     if (acpi_cfg != NULL) {
    //         cJSON *dir_json = cJSON_GetObjectItem(acpi_cfg,
    //         "acpi_tables_dir"); if (dir_json == NULL ||
    //         !cJSON_IsString(dir_json)) {
    //             log_error("acpi_config requires string field
    //             'acpi_tables_dir'"); goto err_out;
    //         }
