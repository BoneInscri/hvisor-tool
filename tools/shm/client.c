...
static int32_t client_init(struct Client *raw_client, uint32_t remote_zone_id) {
    ...
    // ASSERT(raw_client->init_mark != INIT_MARK_DESTORYED);   //
    // 防止销毁后再次初始化
    ...
    struct Channel *target_channel =
        channel_ops.target_channel_get(remote_zone_id);
    ...
    if (target_channel == NULL) /* 没有获取到目标核心的通道 */
    {
        ...
        // printf("client_init_info: get target channel success =
        // %u\n",remote_zone_id);
    }
    ...
    if (remote_zone_id == ZONE_NPUcore_ID) {
        ...
        if (mem_fd >= 0) {
            ...
            dst_queue_virt =
                mmap(NULL, 0x1000, PROT_READ | PROT_WRITE, MAP_SHARED, mem_fd,
                     0x7e410000); // zonex_ram_ipa
    ...
    /* 检查目标通道是否已经被启用 */
    {
        ...
        if (dst_queue_virt != NULL) {
            // 检查目标zone的消息队列状态
            struct AmpMsgQueue *dst_queue =
                (struct AmpMsgQueue *)dst_queue_virt;
        ...
        if (retry_count == 0) {
            printf("Channel not ready, waiting for Non-Root Linux "
                   "initialization...\n");
        }
    ...
    //!!! reuse the msg_queue->working_mark instead of
    //!msg_queue_mutex->msg_queue_mark
...
static struct Msg *client_empty_msg_get(struct Client *client,
                                        uint32_t remote_service_id) {
...
static int32_t client_empty_msg_put(struct Client *client,
...
static int32_t client_msg_send_and_notify(struct Client *client,
                                          struct Msg *full_msg) {
...
static void *client_shm_malloc(struct Client *client, uint32_t size,
                               enum MallocType type) {
