...
const int default_qos_profiles_count =
    sizeof(default_qos_profiles) / sizeof(default_qos_profiles[0]);
...
static int qos_init_impl(struct QoSChannelController *qos_ctrl,
    ...
    qos_ctrl->queues[QOS_REALTIME].capacity =
        (queue_slots * 25 / 100) < MAX_QOS_QUEUE_SIZE ? (queue_slots * 25 / 100)
                                                      : MAX_QOS_QUEUE_SIZE;
    ...
    qos_ctrl->queues[QOS_THROUGHPUT].capacity =
        (queue_slots * 40 / 100) < MAX_QOS_QUEUE_SIZE ? (queue_slots * 40 / 100)
                                                      : MAX_QOS_QUEUE_SIZE;
    ...
    qos_ctrl->queues[QOS_RELIABLE].capacity =
        (queue_slots * 25 / 100) < MAX_QOS_QUEUE_SIZE ? (queue_slots * 25 / 100)
                                                      : MAX_QOS_QUEUE_SIZE;
    ...
    qos_ctrl->queues[QOS_BEST_EFFORT].capacity =
        (queue_slots * 10 / 100) < MAX_QOS_QUEUE_SIZE ? (queue_slots * 10 / 100)
                                                      : MAX_QOS_QUEUE_SIZE;
...
static int allocate_buffer_impl(struct QoSChannelController *qos_ctrl,
    ...
    if (queue->reserved_buffer >= size) {
        ...
        /* 方案3: 真正的资源耗尽 */
        printf("[QoS] ERROR: Out of buffer (requested: %u, reserved: %u, "
               "available: %u)\n",
...
static int update_metrics_impl(struct QoSMetrics *metrics,
    ...
    metrics->avg_latency_us =
        metrics->total_latency_us / metrics->total_messages;
    ...
    metrics->jitter_us =
        (metrics->jitter_us * 7 + deviation) / 8; /* 指数平滑 */
    ...
    if (time_window_us > 0) {
        metrics->throughput_bps =
            (metrics->total_bytes * 8 * 1000000ULL) / time_window_us;
    }
    ...
    metrics->packet_loss_rate =
        (float)metrics->failed_count / metrics->sent_count;
...
static int check_qos_violation_impl(struct QoSMetrics *metrics,
    ...
    if (metrics->avg_latency_us > profile->max_latency_us) {
        ...
        violations++;
        printf("[QoS] VIOLATION: Latency exceeded - avg: %lu us, max allowed: "
               "%u us\n",
    ...
    if (bandwidth_mbps < profile->min_bandwidth_mbps) {
        ...
        violations++;
        printf("[QoS] VIOLATION: Bandwidth below minimum - current: %lu Mbps, "
               "min: %u Mbps\n",
    ...
    if (metrics->packet_loss_rate > profile->loss_tolerance) {
        violations++;
        printf("[QoS] VIOLATION: Packet loss exceeded - current: %.2f%%, max: "
               "%.2f%%\n",
...
static void print_qos_stats_impl(struct QoSChannelController *qos_ctrl) {
    ...
    const char *qos_class_names[] = {"REALTIME", "THROUGHPUT", "RELIABLE",
                                     "BEST_EFFORT"};
...
/* ========== 操作接口导出 ========== */
struct QoSOperations qos_ops = {.qos_init = qos_init_impl,
