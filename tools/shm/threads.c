...
void *worker(void *arg) {
    ...
    Task *task;
    struct Msg *msg =
        client_ops.empty_msg_get(amp_client, 0); // modify service_id later
    if (msg == NULL) {
...
void adjust_thread_pool(ThreadPool *pool, int new_num_threads) {
    ...
    pool->threads = (pthread_t *)realloc(pool->threads,
                                         sizeof(pthread_t) * new_num_threads);
    if (pool->threads == NULL) {
...
void handle_request(void *arg1, void *arg2) {
    ...
    printf("request : %u, id: %u, data_string: %s, size: %u, output_dir: %s\n",
           request->request_id, request->service_id, request->data_string,
           request->size, request->output_dir);
    ...
    // int ret = general_safe_service_request(request->client, msg,
    // request->request_id,
