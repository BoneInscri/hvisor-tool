...
/* core_cfg.h */
struct ZoneInfo zone_infos[] = {{/* 0: Root Linux 节点 */
...
/* shm_cfg.h */
struct ShmCfg shm_cfgs[] = {{/* Linux  共享内存配置信息 */
...
static struct ShmCfg *shm_cfg_get_by_id(uint16_t target_id) {
    ...
    for (i = 0; shm_cfgs[i].zone != NULL; i++) {
        if (shm_cfgs[i].zone->id == target_id) {
            // printf("shm_cfg_get_by_id_info: find shm cfg success, target id =
            // %u\n", target_id);
            return &shm_cfgs[i];
        }
    }
...
struct ShmCfgOps shm_cfg_ops = {.get_by_id = shm_cfg_get_by_id};
