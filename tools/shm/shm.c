...
static uint64_t malloc_from_volatile_block(int start_index, int end_index,
                                           int size) {
    for (int i = start_index; i < end_index; i++) {
        if (shm.shm_infos->vblock_infos[i].state ==
            VBLOCK_STATE_ENABLE) /* 当前区块还可以尝试继续进行分配 */
        {
            if (size <=
                shm.shm_infos->vblock_infos[i]
                    .available_size) /* 当前块可以满足此次内存分配请求 */
            {

                int32_t result =
                    shm.shm_infos->vblock_infos[i].next_alloc_offset;
...
#if UNUSED_COUNT
                shm.unused_mem_size += shm.shm_infos->vblock_infos[i]
                                           .available_size; /* 内存碎片 */
#endif                                                      /* UNUSED_COUNT */
                shm.shm_infos->vblock_infos[i].state =
                    VBLOCK_STATE_DISENABLE; /* 当前块弃用，开始启用下一块 */
            }
        } else /* 查看当前块能不能重新加入分配 */
        {
            if (shm.shm_infos->vblock_infos[i].free_count ==
                shm.shm_infos->vblock_infos[i].alloc_count) {
                shm.shm_infos->vblock_infos[i].next_alloc_offset =
                    shm.shm_infos->vblock_infos[i].alloc_start_offset;
                shm.shm_infos->vblock_infos[i].available_size =
                    shm.shm_infos->vblock_infos[i].total_length;
...
static uint64_t shm_addr_to_offset(void *ptr) {
    ...
    if (ptr < shm.shm_zone_start ||
        ptr > shm.shm_zone_start + shm.shm_cfg->zone_shm->len) {
        printf("shm_addr_to_offset_error: ptr out of range, ptr = %p, "
               "shm.shm_zone_start = %p, shm.shm_zone_end = %p\n",
               ptr, shm.shm_zone_start,
               (void *)(shm.shm_zone_start + shm.shm_cfg->zone_shm->len));
        while (1) {
        }
    }
...
static void *shm_offset_to_addr(uint32_t offset) {
    ...
    if (offset > shm.shm_cfg->zone_shm->len) {
        printf("shm_offset_to_addr_error: error range, offset = %u, "
               "shm.shm_cfg->zone_shm->len = %u\n",
...
static int32_t shm_init(void) {
    ...
    struct ShmCfg *shm_cfg = shm_cfg_ops.get_by_id(
        zone_info->id); /* 获取当前核心的共享内存配置信息 */
    if (shm_cfg == NULL) {
    ...
    if (shm_cfg->zone_shm == NULL) /* 当前节点没有共享内存 */
    {
        // shm_init_mark = INIT_MARK_DESTORYED;
        // TODO: release the mutex
        printf("shm_init error: must have total shm addr info even have no "
               "core shm\n");
        while (1) {
        }
    }
    ...
    shm.shm_infos = (struct ShmInfos *)mmap(
        NULL, MEM_PAGE_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, hvisor_fd,
        shm_infos_pa); // use hvisor_mmap handler
    ...
    printf("shm_init_info: mmap shm manage info success [shm_infos_pa = %p, "
           "shm_infos_va = %p]\n",
    ...
    //     if (atomic_load(&shm.shm_infos->block_init_mark) ==
    //     INIT_MARK_INITIALIZED) /* 在整个节点内存探测是否已经被初始化过了 */
    //     {
    ...
    //     uint32_t except_state = INIT_MARK_RAW_STATE; /*
    //     假定该共享内存还未被初始化 */
    //     /* 试图获取初始化权 */
    //     if
    //     (!atomic_compare_exchange_strong(&shm.shm_infos->block_init_mark,&except_state,INIT_MARK_INITIALIZING))
    //     {
    //         while (1)
    //         {
    //             uint8_t init_state =
    //             atomic_load(&shm.shm_infos->block_init_mark); if (init_state
    //             == INIT_MARK_INITIALIZING)
    //             {
    ...
    //                 printf("shm_init_error: init shm error by others or init
    //                 mark error\n"); return -1;
    //             }
    //         }
    //     }
    ...
    uint32_t pblock_size =
        (shm_cfg->pblock_size > min_block_size) ? shm_cfg->pblock_size : 0;
    ...
    uint32_t vblock_size =
        shm_cfg->vblock_size > min_block_size ? shm_cfg->vblock_size : 0;
    ...
    if (vblock_size > 0 && SHM_VBLOCK_CNT > 0) {
        ...
        if (vblock_each_size < min_block_size) /* 查看内存配置是否合理 */
        {
            ...
            printf("shm_init_error: each vblock is too small vblock_each_size "
                   "=%u, min_block_size = %u, vblock_size = %u, vblock_count = "
                   "%u\n",
                   vblock_each_size, min_block_size, vblock_size,
                   SHM_VBLOCK_CNT);
            while (1) {
            }
        }
    ...
    // printf("shm_init_info: pblock_size = %u KB, vblock_size = %u KB,
    // vblock_each_size = %u KB, min_block_size = %u B\n",
    //         pblock_size / KB, vblock_size / KB, vblock_each_size / KB,
    //         min_block_size);
    // /* 初始化互斥标记 */
    ...
    if (vblock_each_size > min_block_size) /* 存在短期区 */
    {
        ...
        if (init_offset != zone_shm_end - zone_shm_start) {
            ...
            // mark_flag_ops.unlock(&shm_init_lock);
            printf(
                "shm_init_error: zone_shm_end = 0x%lx, init_offset = 0x%lx\n",
    ...
    printf("shm_init_success: init shm success\n");
    printf("shm_init_summary: zone_start=%p, zone_length=0x%x, "
           "pblock_size=0x%x, vblock_size=0x%x\n",
           shm.shm_zone_start, zone_shm_length, shm.shm_cfg->pblock_size,
           shm.shm_cfg->vblock_size);
    return 0;
}
...
static void *shm_malloc(uint32_t size, enum MallocType type) {
    ...
    /* 按照配置信息进行字节对齐 */
    size =
        (size + shm.shm_cfg->bit_align - 1) & (~(shm.shm_cfg->bit_align - 1));

    printf("shm_malloc_debug: size alignment [original=%d, aligned=%d, "
           "bit_align=%d]\n",
           original_size, size, shm.shm_cfg->bit_align);
    switch (type) {
    case MALLOC_TYPE_P: {
        byte_flag_ops.lock(&shm.shm_infos->pblock_lock); /* 长期区锁 */
        if (size > shm.shm_infos->pblock_info
                       .available_size) /* 申请的内存大小超过了最大内存限制 */
        {
            byte_flag_ops.unlock(&shm.shm_infos->pblock_lock); /* 解锁 */
            printf("shm_malloc_warn: the size is large than max avi size, size "
                   "= %d, avi size = %d\n",
                   size, shm.shm_infos->pblock_info.available_size);
            return NULL;
        }
        void *result = shm.shm_zone_start +
                       shm.shm_infos->pblock_info
                           .next_alloc_offset; /* 记录下需要返回的地址 */

        printf("shm_malloc_debug: MALLOC_TYPE_P [zone_start=%p, offset=%d, "
               "final_addr=%p, size=%d]\n",
               shm.shm_zone_start, shm.shm_infos->pblock_info.next_alloc_offset,
               result, size);
        printf("shm_malloc_debug: pblock info [next_alloc_offset=%d, "
               "available_size=%d]\n",
               shm.shm_infos->pblock_info.next_alloc_offset,
               shm.shm_infos->pblock_info.available_size);
        shm.shm_infos->pblock_info.available_size -=
            size; /* 减去已经被分配的空间大小 */
        shm.shm_infos->pblock_info.next_alloc_offset +=
            size; /* 将待分配地址指向下一个位置 */
        ...
        // printf("shm_malloc_success: MALLOC_TYPE_P [start address: %p, length
        // = %d B]\n",
        //     result, size);
        return result;
    }
    case MALLOC_TYPE_V: {
        ...
        uint32_t current_volatile_block_index =
            shm.shm_infos
                ->vblock_current; /* 记录开始查找前短期块的当前位置下标 */
        int32_t result = malloc_from_volatile_block(
            current_volatile_block_index, SHM_VBLOCK_CNT,
            size);      /* 从当前位置往后找 */
        if (result < 0) /* 没找到 */
        {
            result = malloc_from_volatile_block(0, current_volatile_block_index,
                                                size); /* 从前往当前位置找 */
        }
        ...
        if (shm.shm_infos->vblock_current < SHM_VBLOCK_CNT) {
            ...
            printf(
                "shm_malloc_debug: block[%d] info [start_offset=%d, "
                "available_size=%d, total_length=%d]\n",
                current_block,
...
static int32_t shm_destory(void) {
    ...
    munmap(shm.shm_zone_start,
           shm.shm_cfg->zone_shm->len); /* 将连接到进程的 MMAP 内存解绑 */
...
static void shm_free(void *ptr) {
    ...
    uint32_t block_index =
        data_offset /
        shm.shm_infos->vblock_each_size; /* 读取短期块的控制信息 */
...
struct ShmOps shm_ops = {.addr_to_offset = shm_addr_to_offset,
