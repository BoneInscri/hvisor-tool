...
static int32_t msg_queue_mutex_init(struct MsgQueueMutex *mutex,
                                    uint16_t msg_queue_buf_size) {
    // /* 是否已经被初始化过了 */
    ...
    // */ uint32_t aim_mark = INIT_MARK_INITIALIZING;   /*
    // 期望将其切换为初始化中 */

    /* 尝试获取互斥体的初MSG_QUEUE_MARK_IDLE始化权 */
    // if (!atomic_compare_exchange_strong(&mutex->mutex_init_mark,
    // &expected_mark, aim_mark))
    // {
    //     /* 当前互斥体的状态不是初始状态 */
    //     while (1)
    //     {
    ...
    //         if (now_mark == INIT_MARK_INITIALIZING) /*
    //         其他任务正在初始化：等待 */
    //         {
    //             LOGINFO("msg_queue_mutex_init_info: waiting others task init
    //             over\n");
    //         }
    //         else if (now_mark == INIT_MARK_INITIALIZED) /*
    //         其他任务已经初始化完毕 */
    //         {
    ...
    //         {
    //             INFOS("msg_queue_mutex_init_error: mutex init fail by others
    //             before\n"); return -1;
    //         }
    ...
    /* 获取到了互斥体的初始化权 */
    // atomic_store(&mutex->msg_queue_mark, MSG_QUEUE_MARK_IDLE); /*
    // 目标核心空闲 */
...
int32_t msg_queue_mutex_is_init(struct MsgQueueMutex *mutex) {
    ...
    // return atomic_load(&mutex->mutex_init_mark) == INIT_MARK_INITIALIZED ? 0
    // : -1;
    return -1;
}
...
static int32_t msg_queue_init(struct AmpMsgQueue *msg_queue, uint32_t mem_len) {
    ...
    if (mem_len <
        infos_length + buf_length) /* 消息队列至少要可以放入一个消息 */
    {
        printf("msg_queue_init_error: memmory size too small = %u, at least = "
               "%u\n",
    ...
    uint32_t buf_size =
        (mem_len - infos_length) / buf_length; /* 计算可用的消息个数 */

    if (buf_size > MSG_QUEUE_MAX_ENTRY_CNT) {
        printf("msg_queue_init_error: too much msg entry size = %u, max = %u, "
               "check it\n",
    ...
    // printf("buffer_queue_init_success: mem_len = %u, infos_length = %u,
    // buf_length = %u, buf_size = %u, unused = %u\n",
...
static int32_t msg_queue_is_ready(struct AmpMsgQueue *msg_queue) {
    ...
    // msg_queue->working_mark); return msg_queue->working_mark ==
    // INIT_MARK_INITIALIZED ? 0 : -1; // need NPUCore set it!
    if (msg_queue->working_mark == INIT_MARK_INITIALIZED ||
        msg_queue->working_mark == MSG_QUEUE_MARK_IDLE) {
        return 0;
    } else {
        return -1;
    }
...
static uint16_t msg_queue_pop(struct AmpMsgQueue *msg_queue, uint16_t *head) {
    ...
    if (*head >= msg_queue->buf_size) {
        /* 链空 */
        // printf("msg_queue_pop_warn: have no more msg, end = %u,
        // msg_queue->buf_size = %u\n",
...
static int32_t msg_queue_push(struct AmpMsgQueue *msg_queue, uint16_t *head,
                              uint16_t msg_index) {
    ...
    {
        *head = msg_index;
        msg_queue->entries[msg_index].nxt_idx =
            msg_queue->buf_size; /* 防止意外链接 */
    } else {
        /* 链表非空 */
...
struct MsgQueueOps msg_queue_ops = {.init = msg_queue_init,
