...
static void byte_flag_lock(ByteFlag *flag) {
    uint8_t expected_state =
        BYTE_UNLOCK_STATE; /* 只有处于未加锁状态的锁才能被加锁 */
...
static int32_t byte_flag_unlock(ByteFlag *flag) {
    uint8_t expected_state =
        BYTE_LOCKED_STATE; /* 只有处于加锁状态的锁才能被解锁 */
...
struct ByteFlagOps byte_flag_ops = {.init = byte_flag_init,
...
static void mark_flag_unlock(MarkFlag *flag) { atomic_flag_clear(flag); }
...
struct MarkFlagOps mark_flag_ops = {.lock = mark_flag_lock,
