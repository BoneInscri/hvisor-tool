 ...
 * 占 2MB CH1: TX=0xC0200000(16KB), RX=0xC0204000(16KB),
 * Data=0xC0208000(1MB-32KB)  → 占 1MB CH2: TX=0xC0300000(16KB),
 * RX=0xC0304000(16KB), Data=0xC0308000(1MB-32KB)  → 占 1MB 总占用: 0xC0000000 ~
 * 0xC0400000 = 4MB
 */
...
#define DEFAULT_BLOCK_SIZE                                                     \
    4096 // 与 HighSpeedCProxy 的 HSNET_MEM_BLOCK_SIZE 一致
...
typedef struct {
    ...
    HyperampChannel
        ch[HYPERAMP_NUM_CHANNELS]; // ch[0]=ch1, ch[1]=ch2, ch[2]=ch3
...
static volatile void *map_one_region(int fd, uint64_t phys_addr, size_t size,
    ...
    void *mapped =
        mmap(NULL, map_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, phys_addr);
...
static int map_physical_memory(uint64_t phys_addr, size_t size) {
    ...
    for (int c = 0; c < HYPERAMP_NUM_CHANNELS; c++) {
        ...
        ch->tx_queue = (volatile HyperampShmQueue *)map_one_region(
            g_ctx.fd_mem, ch->tx_phys_addr, ch->tx_phys_size, &ch->tx_map_base,
            &ch->tx_map_size);
        if (!ch->tx_queue)
            goto err;
        ...
        ch->rx_queue = (volatile HyperampShmQueue *)map_one_region(
            g_ctx.fd_mem, ch->rx_phys_addr, ch->rx_phys_size, &ch->rx_map_base,
            &ch->rx_map_size);
        if (!ch->rx_queue)
            goto err;
        ...
        for (int s = 0; s < ch->data_seg_cnt; s++) {
            ...
            seg->virt_addr =
                map_one_region(g_ctx.fd_mem, seg->phys_addr, seg->phys_size,
        ...
        /* 兼容旧代码：指向第一段 */
        ch->data_region =
            (ch->data_seg_cnt > 0) ? ch->data_segs[0].virt_addr : NULL;
        ch->data_phys_addr =
            (ch->data_seg_cnt > 0) ? ch->data_segs[0].phys_addr : 0;
        ch->data_phys_size = ch->data_total_size;

        printf("[HyperAMP] ch%d: TX=%p(0x%lx) RX=%p(0x%lx) Data=%p(%zu bytes, "
               "%d segs)\n",
               c + 1, (void *)ch->tx_queue, ch->tx_phys_addr,
               (void *)ch->rx_queue, ch->rx_phys_addr, (void *)ch->data_region,
               ch->data_total_size, ch->data_seg_cnt);
    }
...
int hyperamp_linux_init(uint64_t phys_addr, int is_creator) {
    ...
    printf("[HyperAMP]   TX Queue:    %p (phys: 0x%lx)\n", g_ctx.tx_queue,
           g_ctx.ch[g_ctx.active_ch].tx_phys_addr);
    printf("[HyperAMP]   RX Queue:    %p (phys: 0x%lx)\n", g_ctx.rx_queue,
           g_ctx.ch[g_ctx.active_ch].rx_phys_addr);
    ...
    if (is_creator) {
        ...
        /* CONNECTOR 模式：seL4 已初始化队列，Linux 侧以 is_creator=0
         * 注册自己的虚拟地址， 并对全部 3 个 channel 执行
         * hyperamp_queue_init(queue, cfg, 0)。 */
        ...
        for (int c = 0; c < HYPERAMP_NUM_CHANNELS; c++) {
            ...
            uint16_t tx_cap = hyperamp_safe_read_u16(
                ch->tx_queue, offsetof(HyperampShmQueue, capacity));
            uint16_t rx_cap = hyperamp_safe_read_u16(
                ch->rx_queue, offsetof(HyperampShmQueue, capacity));
    ...
    // 安全读取队列信息（逐字节）
    uint32_t tx_magic = hyperamp_safe_read_u32(
        g_ctx.tx_queue, offsetof(HyperampShmQueue, magic));
    uint16_t tx_capacity = hyperamp_safe_read_u16(
        g_ctx.tx_queue, offsetof(HyperampShmQueue, capacity));
    uint16_t tx_block_size = hyperamp_safe_read_u16(
        g_ctx.tx_queue, offsetof(HyperampShmQueue, block_size));

    uint32_t rx_magic = hyperamp_safe_read_u32(
        g_ctx.rx_queue, offsetof(HyperampShmQueue, magic));
    uint16_t rx_capacity = hyperamp_safe_read_u16(
        g_ctx.rx_queue, offsetof(HyperampShmQueue, capacity));
    uint16_t rx_block_size = hyperamp_safe_read_u16(
        g_ctx.rx_queue, offsetof(HyperampShmQueue, block_size));
 ...
 */
static int _send_on_ch(int ch_idx, uint8_t msg_type, uint16_t frontend_sess_id,
    ...
    CACHE_INVALIDATE(ch->tx_queue);
    uint16_t cap = hyperamp_safe_read_u16(ch->tx_queue,
    ...
    int ret = hyperamp_queue_enqueue(ch->tx_queue, ZONE_ID_LINUX, msg_buf,
                                     total_len, ch->data_region);
    if (ret == HYPERAMP_OK) {
        g_ctx.tx_count++;
        printf("[HyperAMP] ch%d TX: type=%u sess=%u/%u len=%u\n", ch_idx + 1,
               msg_type, frontend_sess_id, backend_sess_id, payload_len);
    } else {
        g_ctx.tx_errors++;
 ...
 */
int hyperamp_linux_send(uint8_t msg_type, uint16_t frontend_sess_id,
                        uint16_t backend_sess_id, const void *payload,
                        uint16_t payload_len) {
 ...
 */
int hyperamp_linux_send_all(uint8_t msg_type, uint16_t frontend_sess_id,
                            uint16_t backend_sess_id, const void *payload,
                            uint16_t payload_len) {
    int ok = 0;
    for (int c = 0; c < HYPERAMP_NUM_CHANNELS; c++) {
        if (_send_on_ch(c, msg_type, frontend_sess_id, backend_sess_id, payload,
 ...
 */
int hyperamp_linux_recv(HyperampMsgHeader *hdr, void *payload,
    ...
    if (rx_capacity == 0) {
        ...
        if (!queue_not_ready_printed) {
            printf("[HyperAMP] RX Queue not initialized yet (capacity=0), "
                   "waiting for seL4...\n");
    ...
    if (!queue_ready_printed) {
        ...
        rx_capacity = hyperamp_safe_read_u16(
            g_ctx.rx_queue, offsetof(HyperampShmQueue, capacity));

        printf("[HyperAMP] ✓ RX Queue initialized (capacity=%u), ready to "
               "receive!\n",
               rx_capacity);

        // 关键：打印前再次失效缓存，确保读取的是最新数据而不是 CPU
        // 缓存中的旧数据
        CACHE_INVALIDATE(g_ctx.rx_queue);
    ...
    int ret =
        hyperamp_queue_dequeue(g_ctx.rx_queue, ZONE_ID_LINUX, msg_buf,
                               sizeof(msg_buf), &actual_len, rx_data_base);
    if (ret != HYPERAMP_OK) {
 ...
 * @brief 发送原始数据消息 (便捷函数)
 */
int hyperamp_linux_send_data(uint16_t session_id, const void *data,
                             uint16_t data_len) {
    return hyperamp_linux_send(HYPERAMP_MSG_TYPE_DATA, session_id, 0, data,
                               data_len);
}

int hyperamp_linux_send_data_all(uint16_t session_id, const void *data,
                                 uint16_t data_len) {
    return hyperamp_linux_send_all(HYPERAMP_MSG_TYPE_DATA, session_id, 0, data,
                                   data_len);
}

int hyperamp_linux_call_service(uint16_t service_id, const void *data,
                                uint16_t data_len) {
    return hyperamp_linux_send(HYPERAMP_MSG_TYPE_SERVICE, service_id, 0, data,
                               data_len);
}

int hyperamp_linux_call_service_all(uint16_t service_id, const void *data,
                                    uint16_t data_len) {
    return hyperamp_linux_send_all(HYPERAMP_MSG_TYPE_SERVICE, service_id, 0,
                                   data, data_len);
}
...
int hyperamp_linux_has_message(void) {
    ...
    // 安全读取 header 和 tail
    uint16_t header = hyperamp_safe_read_u16(
        g_ctx.rx_queue, offsetof(HyperampShmQueue, header));
    uint16_t tail = hyperamp_safe_read_u16(g_ctx.rx_queue,
...
void hyperamp_linux_get_status(void) {
    ...
    // 安全读取 TX 队列状态
    uint16_t tx_header = hyperamp_safe_read_u16(
        g_ctx.tx_queue, offsetof(HyperampShmQueue, header));
    uint16_t tx_tail = hyperamp_safe_read_u16(g_ctx.tx_queue,
                                              offsetof(HyperampShmQueue, tail));
    uint16_t tx_capacity = hyperamp_safe_read_u16(
        g_ctx.tx_queue, offsetof(HyperampShmQueue, capacity));
    uint32_t tx_enqueue = hyperamp_safe_read_u32(
        g_ctx.tx_queue, offsetof(HyperampShmQueue, enqueue_count));
    uint32_t tx_dequeue = hyperamp_safe_read_u32(
        g_ctx.tx_queue, offsetof(HyperampShmQueue, dequeue_count));

    printf("[HyperAMP]   header: %u, tail: %u, capacity: %u\n", tx_header,
           tx_tail, tx_capacity);

    // 计算队列长度
    uint16_t tx_length = (tx_header >= tx_tail)
                             ? (tx_header - tx_tail)
                             : (tx_capacity - tx_tail + tx_header);
    printf("[HyperAMP]   length: %u, enqueued: %u, dequeued: %u\n", tx_length,
           tx_enqueue, tx_dequeue);

    // 读取锁状态
    size_t tx_lock_offset = offsetof(HyperampShmQueue, queue_lock);
    uint32_t tx_lock_value = hyperamp_safe_read_u32(
        g_ctx.tx_queue,
        tx_lock_offset + offsetof(HyperampSpinlock, lock_value));
    uint32_t tx_lock_owner = hyperamp_safe_read_u32(
        g_ctx.tx_queue,
        tx_lock_offset + offsetof(HyperampSpinlock, owner_zone_id));
    uint32_t tx_lock_count = hyperamp_safe_read_u32(
        g_ctx.tx_queue,
        tx_lock_offset + offsetof(HyperampSpinlock, lock_count));
    uint32_t tx_lock_contention = hyperamp_safe_read_u32(
        g_ctx.tx_queue,
        tx_lock_offset + offsetof(HyperampSpinlock, contention_count));
    ...
    // 安全读取 RX 队列状态
    uint16_t rx_header = hyperamp_safe_read_u16(
        g_ctx.rx_queue, offsetof(HyperampShmQueue, header));
    uint16_t rx_tail = hyperamp_safe_read_u16(g_ctx.rx_queue,
                                              offsetof(HyperampShmQueue, tail));
    uint16_t rx_capacity = hyperamp_safe_read_u16(
        g_ctx.rx_queue, offsetof(HyperampShmQueue, capacity));
    uint32_t rx_enqueue = hyperamp_safe_read_u32(
        g_ctx.rx_queue, offsetof(HyperampShmQueue, enqueue_count));
    uint32_t rx_dequeue = hyperamp_safe_read_u32(
        g_ctx.rx_queue, offsetof(HyperampShmQueue, dequeue_count));

    printf("[HyperAMP]   header: %u, tail: %u, capacity: %u\n", rx_header,
           rx_tail, rx_capacity);

    uint16_t rx_length = (rx_header >= rx_tail)
                             ? (rx_header - rx_tail)
                             : (rx_capacity - rx_tail + rx_header);
    printf("[HyperAMP]   length: %u, enqueued: %u, dequeued: %u\n", rx_length,
           rx_enqueue, rx_dequeue);

    // 读取锁状态
    size_t rx_lock_offset = offsetof(HyperampShmQueue, queue_lock);
    uint32_t rx_lock_value = hyperamp_safe_read_u32(
        g_ctx.rx_queue,
        rx_lock_offset + offsetof(HyperampSpinlock, lock_value));
    uint32_t rx_lock_owner = hyperamp_safe_read_u32(
        g_ctx.rx_queue,
        rx_lock_offset + offsetof(HyperampSpinlock, owner_zone_id));
    uint32_t rx_lock_count = hyperamp_safe_read_u32(
        g_ctx.rx_queue,
        rx_lock_offset + offsetof(HyperampSpinlock, lock_count));
    uint32_t rx_lock_contention = hyperamp_safe_read_u32(
        g_ctx.rx_queue,
        rx_lock_offset + offsetof(HyperampSpinlock, contention_count));
...
static void parse_global_addr(char *json_path) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (strstr(flag, "ch1"))
            ch_idx = 0;
        else if (strstr(flag, "ch2"))
            ch_idx = 1;
        else if (strstr(flag, "ch3"))
            ch_idx = 2;
        if (ch_idx < 0)
            continue;
    ...
    for (int c = 0; c < HYPERAMP_NUM_CHANNELS; c++) {
        ...
        printf("[Info] ch%d: TX=0x%lx(%zu) RX=0x%lx(%zu) data_segs=%d\n", c + 1,
               ch->tx_phys_addr, ch->tx_phys_size, ch->rx_phys_addr,
               ch->rx_phys_size, ch->data_seg_cnt);
...
static void print_usage(const char *prog) {
    ...
    printf("Options:\n");
    printf("  -c          Create/initialize queues (default: connect to "
           "existing)\n");
 ...
 * @param max_size 最大允许文件大小 (Bulk模式使用BULK_BUFFER_SIZE,
 * 普通模式使用HYPERAMP_MSG_MAX_SIZE)
 * @return 0 成功, -1 失败
 */
 ...
 * @return 0 成功, -1 失败
 */
static int write_file(const char *filename, const uint8_t *data,
                      size_t data_len) {
...
int hyperamp_linux_bulk_transfer(const void *input_data, size_t input_len,
    ...
    if (input_len > BULK_BUFFER_SIZE) {
        printf(
            "[HyperAMP] Error: Data too large for Bulk Transfer (%zu > %d)\n",
    ...
    volatile void *bulk_buf_ptr =
        (volatile void *)((uintptr_t)g_ctx.data_region + BULK_BUFFER_OFFSET);
    ...
    // 3. 发送消息 (Payload 为描述符)
    int ret =
        hyperamp_linux_send(HYPERAMP_MSG_TYPE_BULK, 0, 0, &desc, sizeof(desc));
    if (ret != HYPERAMP_OK) {
    ...
    ret = hyperamp_linux_recv(&hdr, &resp_desc, (uint16_t)sizeof(resp_desc),
                              &actual_pl_len);
    if (ret != HYPERAMP_OK) {
    ...
    // 6. 读取结果 (从共享内存读回)
    // 注意: seL4 可能处理后的数据长度不变，也可能变化。这里简单假设长度一致或是
    // header 中的 length
    size_t result_len = resp_desc.length;
    if (result_len > BULK_BUFFER_SIZE)
        result_len = BULK_BUFFER_SIZE;

    // Memory copy from Shared Memory
    volatile uint8_t *bulk_src =
        (volatile uint8_t *)((uintptr_t)g_ctx.data_region + resp_desc.offset);
    ...
    *output_len = result_len;
    printf(
        "[HyperAMP] Bulk Transfer Complete: Read %zu bytes from offset 0x%x\n",
...
int main(int argc, char *argv[]) {
    ...
    if (shm_json_path) {
        ...
        static const uint64_t ch_tx[HYPERAMP_NUM_CHANNELS] = {
            SHM_CH0_TX_PADDR, SHM_CH1_TX_PADDR, SHM_CH2_TX_PADDR};
        static const uint64_t ch_rx[HYPERAMP_NUM_CHANNELS] = {
            SHM_CH0_RX_PADDR, SHM_CH1_RX_PADDR, SHM_CH2_RX_PADDR};
        static const uint64_t ch_data[HYPERAMP_NUM_CHANNELS] = {
            SHM_CH0_DATA_PADDR, SHM_CH1_DATA_PADDR, SHM_CH2_DATA_PADDR};
        static const size_t ch_dsz[HYPERAMP_NUM_CHANNELS] = {
            SHM_CH0_DATA_SIZE, SHM_CH1_DATA_SIZE, SHM_CH2_DATA_SIZE};

        for (int c = 0; c < HYPERAMP_NUM_CHANNELS; c++) {
            /* -a 只覆盖 CH0 的 TX 起始地址，其余保持默认 */
            g_ctx.ch[c].tx_phys_addr =
                (c == 0 && phys_addr) ? phys_addr : ch_tx[c];
            g_ctx.ch[c].rx_phys_addr =
                (c == 0 && phys_addr) ? phys_addr + SHM_QUEUE_SIZE : ch_rx[c];
            g_ctx.ch[c].data_phys_addr = (c == 0 && phys_addr)
                                             ? phys_addr + 2 * SHM_QUEUE_SIZE
                                             : ch_data[c];
    ...
    if (do_send && send_msg) {
        ...
        if (send_msg[0] == '@') {
            ...
            size_t max_file_size =
                use_bulk ? BULK_BUFFER_SIZE : HYPERAMP_MSG_MAX_SIZE;
            if (read_file(filename, &file_data, &file_data_len,
                          max_file_size) != 0) {
        ...
        if (service_call_id >= 0) {
            if (use_bulk) {
                ...
                if (use_signed && signature_file) {
                    ...
                    size_t sig_len = 0;
                    if (read_file(signature_file, &sig_data, &sig_len, 256) !=
                        0) {
                    ...
                    if (sig_len < 64 || sig_len > 72) {
                        printf(
                            "Invalid signature length: %zu (expected 64-72)\n",
                            sig_len);
                        free(sig_data);
                        if (need_free && file_data)
                            free(file_data);
                    ...
                    size_t signed_total =
                        sizeof(HyperampSignedHeader) + data_len;
                    ...
                    HyperampSignedHeader *hdr =
                        (HyperampSignedHeader *)final_data;
                    hdr->magic = SIG_MAGIC;
                    hdr->sig_len = (uint16_t)sig_len;
                    hdr->reserved = 0;
                    hdr->payload_len = (uint32_t)data_len;
                    memcpy(hdr->signature, sig_data, sig_len);
                    memcpy(final_data + sizeof(HyperampSignedHeader),
                    ...
                    printf("[HyperAMP] Constructed signed payload: %zu bytes "
                           "(sig=%zu, data=%zu)\n",
                ...
                if (!bulk_response_buf) {
                    ...
                    size_t out_len = 0;
                    if (hyperamp_linux_bulk_transfer(
                            final_data, final_len, bulk_response_buf, &out_len,
                            final_service_id) == HYPERAMP_OK) {
                        printf("\nReceived Bulk response: %zu bytes\n",
                               out_len);
                        if (output_file) {
                            if (write_file(output_file, bulk_response_buf,
                                           out_len) == 0) {
                                printf("✓ Response saved to %s\n", output_file);
                            }
                        } else {
                            printf(
                                "(Output file not specified, skipping save)\n");
                        }
                    } else {
                ...
                printf("\nCalling Service ID %d with %zu bytes (ch: %s)\n",
                       ...
                       target_ch < 0 ? "all"
                                     : (char[4]){(char)('1' + target_ch), 0});
                int send_ret =
                    (target_ch < 0)
                        ? hyperamp_linux_call_service_all(
                              service_call_id, data_to_send, data_len)
                        : hyperamp_linux_call_service(service_call_id,
                                                      data_to_send, data_len);
                    ...
                    if (do_wait || output_file) {
                        ...
                        while (timeout-- > 0 && !received) {
                            if (hyperamp_linux_recv(
                                ...
                                // 保存到文件
                                if (output_file) {
                                    if (write_file(output_file, payload,
                                                   payload_len) == 0) {
                                        printf("✓ Response saved to %s\n",
                                               output_file);
                                    }
    ...
    if (do_recv) {
        ...
        while (hyperamp_linux_recv(&hdr, payload, sizeof(payload) - 1,
                                   &payload_len) == HYPERAMP_OK) {
    ...
    if (do_test) {
        ...
        while (1) {
            ...
            if (line[0] == 'q') {
                ...
                if (hyperamp_linux_recv(&hdr, payload, sizeof(payload) - 1,
                                        &payload_len) == HYPERAMP_OK) {
                    payload[payload_len] = '\0';
                    printf("Received: type=%u, len=%u, data=[%s]\n",
                ...
                printf("Unknown command. Use ? for status, s <msg> to send, r "
                       "to receive, q to quit\n");
            }
        }
    }
