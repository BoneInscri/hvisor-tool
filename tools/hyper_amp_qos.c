 ...
 * @return 0表示成功，-1表示失败
 */
int hyper_amp_qos_client(char *shm_json_path, char *data_input,
                         uint32_t service_id) {
    ...
    if (!qos_initialized) {
        int qos_init_result =
            qos_ops.qos_init(&global_qos_ctrl, 1024 * 1024, 64);
        if (qos_init_result == 0) {
            qos_initialized = 1;
            printf("[QoS-Client] QoS controller initialized (1MB buffer, 64 "
                   "slots)\n");
        } else {
            printf("[Warning] QoS initialization failed (error: %d), "
                   "continuing without QoS\n",
                   qos_init_result);
        }
    }
    ...
    if (data_input[0] == '@') {
        ...
        if (bytes_read != data_size) {
            printf("[Error] Failed to read file completely (read: %zu, "
                   "expected: %u)\n",
    ...
    if (qos_initialized) {
        ...
        printf("  Priority: %s\n", qos_class == QOS_REALTIME     ? "Highest (1)"
                                   ...
                                   : qos_class == QOS_RELIABLE   ? "Medium (3)"
                                                                 : "Low (4)");

        /* P0修复: 资源预留检查 */
        int alloc_result =
            qos_ops.allocate_buffer(&global_qos_ctrl, qos_class, data_size);
        if (alloc_result != 0) {
            printf("[QoS-Warning] Failed to allocate buffer (class: %d, size: "
                   "%u)\n",
                   qos_class, data_size);
    ...
    // msg_send_and_notify() 会执行 transfer(wait_h → proc_ing_h)
    // 并发送中断通知服务端
    ...
    int max_wait = 100;
    int wait_count = 0;
    while (msg->flag.deal_state != MSG_DEAL_STATE_YES &&
           wait_count < max_wait) {
        usleep(10000); // 10ms
        wait_count++;
    }
    ...
    if (qos_initialized) {
         ...
         */
        uint64_t latency_us =
            (recv_time.tv_sec - send_time.tv_sec) * 1000000ULL +
            (recv_time.tv_nsec - send_time.tv_nsec) / 1000;

        printf("\n[QoS-Stats] Performance Metrics:\n");
        printf("  End-to-End Latency: %lu μs (%.2f ms)\n", latency_us,
               latency_us / 1000.0);
        printf("  ⚠️  Note: Latency includes polling overhead (up to "
               "1000ms)\n");
        printf("  Result: %s\n",
               msg->flag.service_result == MSG_SERVICE_RET_SUCCESS ? "SUCCESS"
                                                                   : "FAILED");

        // 检查QoS违规
        struct ServiceQoSProfile *profile =
            qos_ops.get_qos_profile(msg->service_id);
        if (profile && latency_us > profile->max_latency_us) {
            printf("  ⚠️  QoS VIOLATION: Latency exceeded limit (%lu μs > %u "
                   "μs)\n",
        ...
        /* P0修复: 释放预留的资源 */
        int release_result =
            qos_ops.release_buffer(&global_qos_ctrl, qos_class, data_size);
        if (release_result == 0) {
...
int hyper_amp_qos_service(char *shm_json_path) {
    ...
    printf(
        "Initializing QoS-aware server with three-phase batch processing...\n");
    ...
    void *zone0_msg_queue_virt =
        mmap(NULL, addr_infos[1].len, PROT_READ | PROT_WRITE, MAP_SHARED,
             mem_fd, addr_infos[1].start);
    void *zonex_msg_queue_virt =
        mmap(NULL, addr_infos[2].len, PROT_READ | PROT_WRITE, MAP_SHARED,
             mem_fd, addr_infos[2].start);

    if (buf_virt == MAP_FAILED || zone0_msg_queue_virt == MAP_FAILED ||
        zonex_msg_queue_virt == MAP_FAILED) {
    ...
    struct AmpMsgQueue *root_msg_queue =
        (struct AmpMsgQueue *)zone0_msg_queue_addr;
    struct AmpMsgQueue *zonex_msg_queue =
        (struct AmpMsgQueue *)zonex_msg_queue_addr;
    uint64_t root_msg_entries_addr =
        zone0_msg_queue_addr + sizeof(struct AmpMsgQueue);
    ...
    printf("[QoS-Server] Zone0 queue initialized (working_mark=0x%x, "
           "buf_size=%u)\n",
    ...
    printf("[QoS-Server] Zonex queue initialized (working_mark=0x%x, "
           "buf_size=%u)\n",
    ...
    printf("[QoS-Server-DEBUG] Entering main loop (polling mode: %s)\n",
           shm_fd < 0 ? "YES" : "NO");
    printf("[QoS-Server-DEBUG] Initial queue state: proc_ing_h=%u, wait_h=%u, "
           "buf_size=%u\n",
           root_msg_queue->proc_ing_h, root_msg_queue->wait_h,
    ...
    while (running) {
        ...
        if (poll_count % 10 == 0) {
            printf("[QoS-Server-DEBUG] Poll #%d - Queue: proc_ing_h=%u, "
                   "wait_h=%u, working_mark=0x%x, buf_size=%u\n",
                   poll_count, root_msg_queue->proc_ing_h,
                   root_msg_queue->wait_h, root_msg_queue->working_mark,
                   root_msg_queue->buf_size);
            fflush(stdout);
        }
        ...
        if (root_msg_queue->proc_ing_h < root_msg_queue->buf_size ||
            ...
            printf("[QoS-Server-DEBUG] *** MESSAGE DETECTED *** proc_ing_h=%u, "
                   "wait_h=%u, buf_size=%u\n",
                   root_msg_queue->proc_ing_h, root_msg_queue->wait_h,
                   root_msg_queue->buf_size);
        }
        ...
        // 检查是否有消息需要处理
        // 注意: 客户端的 msg_send_and_notify() 已经完成了 transfer(wait_h →
        // proc_ing_h) 所以这里直接检查 proc_ing_h 即可
        if (root_msg_queue->proc_ing_h >= root_msg_queue->buf_size) {
            continue;
        }
        ...
        while (current_head < buf_size && collected_count < MAX_BATCH_SIZE &&
               visited_count < MAX_VISIT) {
            visited_count++;
            ...
            for (int i = 0; i < visited_nodes_count; i++) {
                if (visited_nodes[i] == current_head) {
                    printf("[QoS-Server] ⚠️  CRITICAL: Circular reference "
                           "detected at node %u!\n",
                           current_head);
                    is_duplicate = 1;
                    break;
            ...
            uint64_t msg_entry_addr =
                root_msg_entries_addr + sizeof(struct MsgEntry) * current_head;
            ...
            if (server_qos_initialized) {
                ...
                int enqueue_result =
                    qos_ops.qos_enqueue(&server_qos_ctrl, &qos_msg);
                if (enqueue_result == 0) {
                    collected_count++;
                    printf("  [%d] Enqueued: Service %u, Class: %s, %u bytes\n",
                           ...
                           qos_class == QOS_REALTIME     ? "REALTIME"
                           : qos_class == QOS_THROUGHPUT ? "THROUGHPUT"
                           : qos_class == QOS_RELIABLE   ? "RELIABLE"
                                                         : "BEST_EFFORT",
                           msg->length);
                } else {
                    printf("[QoS-Server] WARNING: Failed to enqueue message "
                           "(queue full?)\n");
                }
            } else {
                collected_count++;
        ...
        if (visited_count >= MAX_VISIT) {
            printf("[QoS-Server] ⚠️  WARNING: Possible circular reference "
                   "detected!\n");
        }
        ...
        while (processed_count < collected_count && server_qos_initialized) {
            ...
            if (scheduled_msg == NULL) {
                printf("[QoS-Server] WARNING: qos_schedule returned NULL (no "
                       "more messages)\n");
                break;
            }
            ...
            if (scheduled_msg->offset >= addr_infos[0].len) {
                ...
                /* P0-CRITICAL-8注意:
                 * 这里dequeue是必要的，因为schedule没有移除消息 */
            ...
            printf("  [%d/%d] Processing: Service %u, Class: %s, Length: %u\n",
                   processed_count + 1, collected_count,
            ...
            // P2修复: 增强的参数验证
            if (scheduled_msg->length > 0 &&
                scheduled_msg->length <= addr_infos[0].len &&
                data_ptr != NULL) {
                switch (scheduled_msg->service_id) {
                case 1: // 加密
                    if (hyperamp_encrypt_service(data_ptr,
                                                 scheduled_msg->length - 1,
                                                 scheduled_msg->length) != 0) {
                        service_result = MSG_SERVICE_RET_FAIL;
                        printf("    Service failed: Encryption error\n");
                    }
                    break;
                case 2: // 解密
                    if (hyperamp_decrypt_service(data_ptr,
                                                 scheduled_msg->length - 1,
                                                 scheduled_msg->length) != 0) {
                        service_result = MSG_SERVICE_RET_FAIL;
                ...
                printf("    ERROR: Invalid message parameters (length: %u, "
                       "data_ptr: %p)\n",
            ...
            // ========== 关键修复: 将处理结果写回原始队列 ==========
            // 问题: scheduled_msg 是 QoS 队列中的副本,客户端轮询的是 Zone0
            // 队列中的原始消息 解决: 找到 Zone0 队列中对应的消息实体,更新其
            // deal_state
            //
            // 方法: 遍历 proc_ing_h 链表,通过 offset/length/service_id
            // 匹配找到原始消息
            uint16_t orig_head = root_msg_queue->proc_ing_h;
            bool found_original = false;

            while (orig_head < buf_size) {
                struct MsgEntry *orig_entry =
                    (struct MsgEntry *)(root_msg_entries_addr +
                                        sizeof(struct MsgEntry) * orig_head);
                ...
                if (orig_msg->offset == scheduled_msg->offset &&
                    ...
                    found_original = true;
                    printf("    ✅ Updated original message state in Zone0 "
                           "queue (index: %u)\n",
                           orig_head);
                    break;
                }
            ...
            if (!found_original) {
                printf("    ⚠️  WARNING: Could not find original message in "
                       "Zone0 queue!\n");
            }

            /* P0-CRITICAL-8修复: 手动dequeue（因为当前schedule不会移除消息）
             * TODO: 未来应该在qos_schedule内部调用dequeue */
            struct QoSMsg *dequeued_msg =
                qos_ops.qos_dequeue(&server_qos_ctrl, qos_class);
            if (dequeued_msg == NULL) {
                printf("[QoS-Server] WARNING: qos_dequeue returned NULL for "
                       "class %d\n",
                       qos_class);
            }
